...
def Cut(data, samples, Fs, basename):                                           # <== idealiter overal zelfdes soort volgorde aanhouden
    ...
    while i < len(start_pulse):
        range_I = start_pulse[i] - 1024/2  # Strat of pulse should be in middle
...
def PlotFoldedCuts(cuttedfilename, basename, N, Fs):
    ...
    for segment in range(1):
        ...
        if len(cuttedfilename[segment*N: (segment+1)*N]) == len(time):
            plt.plot(time[450:-150]/(1e-3), cuttedfilename[segment*N+450: (segment+1)*N-150])       # in ms
            #plt.plot(time[450:-150]*1500, cuttedfilename[segment*N+450: (segment+1)*N-150])         # in m
        #break
    plt.title('\n\nSingle sweep with ringing and reflections', fontsize=24)
    plt.suptitle('used: %s' %(basename), fontsize=9)
    plt.tick_params(axis='both', labelsize=15)
...
def main():
    ...
    for fn in os.listdir('.'):
        if "c_wav10kHz_20170309_config2_DOM_17_sweep10-60kHz_10Vpp_bp1-80.dat" in fn:
            data = np.genfromtxt(fn)
            cutted_data, basename = Cut(data, N, Fs, fn)
            PlotFoldedCuts(cutted_data, basename, N, Fs)
