...
#==============================================================================
    ...
    plt.plot(t1-0.01-(52*(1/2e5)), ijkDOM, linewidth=1, label = 'Config 2')
    t2 = MakeTimeSpan(len(ijkREF), int(2e5))
    plt.plot(t2-0.010015, ijkREF, color = 'r', linewidth=1, label = 'Config 3')
    plt.tick_params(axis='both', labelsize=15)
    plt.title('Reflection pattern wavelet 20kHz\nReference hydrophone', fontsize=24)
    ...
    t1 = MakeTimeSpan(len(ijkDOM), int(2e5))
    ax1.plot(t1, ijkDOM, 'b-', linewidth=1)
    ax1.set_xlabel('time (s)', fontsize = 18)
    # Make the y-axis label, ticks and tick labels match the line color.
    ax1.set_ylabel('ADC', color='b')
    ax1.set_ylabel('amplitude [Pa]\nReference hydrophone c3', color='b', fontsize = 18)
    ...
    ax2 = ax1.twinx()
    t2 = MakeTimeSpan(len(ijkREF), int(2e5))
    ax2.plot(t2, ijkREF, 'r-', linewidth=1)
    ax2.set_ylabel('amplitude [Pa]\nReference hydrophone c2 ', color='r', fontsize = 18)
...
# file to determine calibration value Hydrophone:                                    <== hier bestandsnaam aangepast!
    file_ijkDOM     = 'Time_Conf2Domwavelet20kHzv1.dat'  # .dat
# file to determine calibration value Reference:                                     <== hier bestandsnaam aangepast!
    file_ijkREF     = 'Time_Conf3Barewavelet20kHzv1.dat'              # .dat
