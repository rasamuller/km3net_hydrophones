...
def PlotFoldedCuts(cuttedfilename, basename, N, Fs):
    ...
    time = np.linspace(0, N-1, N)/Fs*1500 - 11.5        
    for segment in range(int(len(cuttedfilename)/N)):
        ...
        if len(cuttedfilename[segment*N: (segment+1)*N]) == len(time):
            ...
            plt.plot(time, cuttedfilename[segment*N: (segment+1)*N]/(1e5))
        break
...
#    Fs = 25e6/128
    ...
    #data, basename = Import_data('Time_Conf2Domwavelet10kHzv1.dat')
    data, basename = Import_data('Time_Conf2Domwavelet20kHzv1.dat')
