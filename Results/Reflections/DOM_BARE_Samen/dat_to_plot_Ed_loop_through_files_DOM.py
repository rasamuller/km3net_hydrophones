...
#==============================================================================
    ...
    plt.plot((t1-0.01)/1e-3, ijkDOM, linewidth=1, label = 'DOM piezo')         # <== ms
#    plt.plot((t1-0.01)*1500, ijkDOM, linewidth=1, label = 'DOM piezo')         # <== m
    t2 = MakeTimeSpan(len(ijkREF), int(25e6/128))
    plt.plot((t2-0.01)/1e-3, ijkREF, color = 'g', linewidth=1, label = 'Bare piezo in box')  # <== ms
#    plt.plot((t2-0.01)*1500, ijkREF, color = 'r', linewidth=1, label = 'Bare piezo in box')   # <== m
#    t2 = MakeTimeSpan(len(ijkREF), int(25e6/128)-7500)
#    plt.plot(t2-0.0103, ijkREF, color = 'r', linewidth=1, label = 'Bare piezo in box')
    plt.tick_params(axis='both', labelsize=15)
    ...
    plt.grid()
    plt.grid('on', 'minor')
    plt.xlabel('Time [ms]', fontsize=18)                                        # <== ms
#    plt.xlabel('Distance [m]', fontsize=18)                                        # <== m
    plt.tight_layout()
    plt.show()
    plt.clf()
...
#==============================================================================
    DOM_val_X = IJk_value(ijkDOM, basenameDOM, int(25e6/128), NFFT, cal_min, 
                          cal_max)  # in [X]
    REF_val_Pa = IJk_value(ijkREF, basenameREF, int(25e6/128), NFFT, cal_min, 
                           cal_max) # in [Pa]
...
# file to determine calibration value Reference:                                     <== hier bestandsnaam aangepast!
    file_ijkREF     = 'c_wav10kHz_20170310_config3_Bare_18_v1_wavelet_20kHz_bp1-80.dat'              # .dat
