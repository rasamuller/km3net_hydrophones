...
def Spectrum(Fs, NFFT, data, basename):
    ...
    if plots == True:
        ...
        print "plot np.sqrt(PSD[:,0])"
        plt.plot(freq, np.sqrt(PSD[:,0]), linewidth=1)
        plt.tick_params(axis='both', labelsize=15)
        plt.title('\nPowerspectrum', fontsize=24)
        plt.suptitle('used: %s\nNFFT: %s'%(basename, NFFT), fontsize=9)
        plt.xlabel('Frequency [Hz]', fontsize=18)
        plt.ylabel('Amplitude gain', fontsize=18)
        plt.tight_layout()
        plt.grid()
        plt.grid('on','minor')
        if save_png == True:
        ...
        print "20*np.log10(np.sqrt(PSD[:,0])"
        plt.plot(freq, 20*np.log10(np.sqrt(PSD[:,0])), linewidth=1)
        plt.tick_params(axis='both', labelsize=15)
        plt.title('\nPowerspectrum', fontsize=24)
        plt.suptitle('used: %s\nNFFT: %s'%(basename, NFFT), fontsize=9)
        plt.xlabel('Frequency [Hz]', fontsize=18)
        plt.ylabel('Voltage gain [dB]', fontsize=18)
        plt.tight_layout()
        plt.grid()
        plt.grid('on','minor')
        if save_png == True:
        ...
        print "20*np.log10(np.sqrt(PSD[:,0])/(1e-6))"
...
def Elec_ratio(wav_file, cal_freq, NFFT, lowcut, highcut):
    ...
    print "plot 1 second of wavio data"
    plt.plot(data[0:Fs])
    plt.show()
    ...
    print "plot 20*np.log10(sqrt_PSD/PSD10kHz)"
    # plot ratios on dB scale
    if plots == True:
        plt.plot(freq, 20*np.log10(ratios), linewidth=1)
        plt.tick_params(axis='both', labelsize=15)
        plt.title('\n\n\nElectronics Powerspectrum\nnormalized at 10kHz', fontsize=24)
        plt.suptitle('Freq at calibration freq %skHz equals one'
                      %int(cal_freq/1000.))
        plt.xlabel('Frequency [Hz]', fontsize=18)
        plt.ylabel('Amplitude gain [dB]', fontsize=18) # only sqrt(Hz) if PDS so NFFT = Fs
        plt.ylim(-20, 20)
        plt.grid()
        plt.grid('on','minor')
        plt.tight_layout()
...
def IJk_value(hyd_data, basename, Fs, NFFT, FcumBegin, FcumEnd):
    # For this PS it doesn't matter that I don't correct for binsize
    # because I will devide these 
    ...
    if plots == True:
        ...
        plt.plot(freq[idxE:idxB-1:-1], 
                 10.*np.log10(abs(PSD[idxE:idxB-1:-1].cumsum())), linewidth=1)
        plt.tick_params(axis='both', labelsize=15)
        str = "$\sigma$ = %6.4f amp" % (sqrt_std)                               # <== general
#        str = "$\sigma$ = %6.4f ACDC counts" % (sqrt_std)                      # <== DOM
#        str = "$\sigma$ = %6.4f Pa" % (sqrt_std)                               # <== REF
        plt.title('\n\nPowerspectrum', fontsize=24)
        plt.suptitle('used: %s\nNFFT: %s\n %s' %(basename, NFFT, str), 
                     fontsize=9)
        plt.ylabel("amp ", fontsize=18)                                         # <== general
#        plt.ylabel('dB re ADC counts', fontsize=18)                            # <== DOM
#        plt.ylabel('dB re Pa', fontsize=18)                                    # <== REF        plt.autoscale(tight = True)
        plt.grid()
        plt.grid('on', 'minor')
        plt.xlabel('Frequency [Hz]', fontsize=18)
        plt.tight_layout()
...
# file to determine calibration value Hydrophone:
    file_ijkDOM     = '20170310_config2_DOM_6_v1_wavelet_10kHz.dat'  # .dat
# file to determine calibration value Reference:
...
# upperbound frequency for cummulative, to determine calibration value:
    ...
    for fn in os.listdir('.'):
        ...
        if ".dat" in fn:
            ...
            # get file --> Use less datapoints for tone and sweep to avoid 
            # OverflowError
            print '- %s' %fn
            basename = os.path.splitext(os.path.basename(fn))[0]
            if 'toon' in fn:
                data = np.genfromtxt(fn, max_rows = Fs/2) #<-- maar een halve seconde ja?!?!
            if 'sweep' in fn:
            ...
            data, basename = butter_bandpass_filter(data, basename, lowcut, 
                                                    highcut, Fs, order=5)
