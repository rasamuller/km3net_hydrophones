...
#################################################
    plt.plot(freq, 10.*np.log10(abs(PSD)), linewidth=2) # PSD in dB re amp^2
    #plot cummulatives
    if cumm == True:
...
def Spectrum(Fs, NFFT, data, basename):
    ...
    if plotjes == True:
        ...
        plt.plot(freq, 20*np.log10(np.sqrt(PSD[:,0])/(1e-6)), 'r', linewidth=2)
        plt.tick_params(axis='both', labelsize=15)
        plt.title('\nWhite noise,\nelectronics only', fontsize=24)
        plt.suptitle('used: %s\nNFFT: %s'%(basename, NFFT), fontsize=9)
        plt.xlabel('Frequency [Hz]', fontsize=18)
...
def main():
    ...
    file_ijkDOM     = '20170310_config2_DOM_6_v1_wavelet_10kHz.dat'  # .dat     # file to determine calibration value Hydrophone
    file_ijkREF     = 'Time_Conf2Domwavelet10kHzv1.dat'              # .dat     # file to determine calibration value Reference
