...
def plot_ERES_N(fig, ax, col, X_MAX, ERES, CL, p_005, p_3s, p_5s, CL_err, p_005_err, p_3s_err, p_5s_err):
    X_MIN = 0
...
#    plt.text(p_005[2]+(p_005[3]-p_005[2])/4.*3., 70, r'$CL$', fontsize=18, weight='normal', bbox=dict(facecolor='white', alpha = 0.5, edgecolor='black', boxstyle='round,pad=1'))
    ax.text(p_005[2]+(p_005[3]-p_005[2])/4.*3. + 100, 50, r'$p = 0.05$', fontsize=20, weight='normal', bbox=dict(facecolor='white', alpha = 0.5, edgecolor='black', boxstyle='round,pad=0.3'))
    ax.text(p_3s[2]+(p_3s[3]-p_3s[2])/4.*2. + 100, 30, r'$3\sigma$', fontsize=20, weight='normal', bbox=dict(facecolor='white', alpha = 0.5, edgecolor='black', boxstyle='round,pad=0.3'))
    ax.text(p_5s[2]+(p_5s[3]-p_5s[2])/4.*1 + 100, 10, r'$5\sigma$', fontsize=20, weight='normal', bbox=dict(facecolor='white', alpha = 0.5, edgecolor='black', boxstyle='round,pad=0.3'))
...
def main():
    ...
    fig, ax1 = plot_ERES_N(fig, ax1, 'green', X_MAX, ERES, TD_CL, TD_p_005, TD_p_3s, TD_p_5s, TD_CL_err, TD_p_005_err, TD_p_3s_err, TD_p_5s_err)
    fig, ax2 = plot_ERES_N(fig, ax2, 'red', X_MAX, ERES, BU_CL, BU_p_005, BU_p_3s, BU_p_5s, BU_CL_err, BU_p_005_err, BU_p_3s_err, BU_p_5s_err)

    ax1.set_title('Number of neutrino detections\nfor significant discovery/exclusion', fontsize=28, y=1.03)
    ax2.set_xlabel('Number of detections \n'+r'between $5 \cdot 10^{20} eV\ <\ E\ <\ 5 \cdot 10^{22}$ eV', fontsize=20)
