...
def Cut(data, samples, Fs, basename):                                           # <== idealiter overal zelfdes soort volgorde aanhouden
    ...
    MIN = np.min(data)                 # Maximum valule in dataset
    treshold = MIN/2.5
...
#            plt.plot(time[450:-150]/(1e-3), cuttedfilename[segment*N+450: (segment+1)*N-150]/(1e5))
        ...
        #break
    plt.title('\n\nWavelets folded piezo in watertight box', fontsize=24)
    plt.suptitle('used: %s' %(basename), fontsize=9)
    plt.tick_params(axis='both', labelsize=15)
...
def main():
    ...
    for fn in os.listdir('.'):
        if "c_wav10kHz_20170310_config3_Bare_16_v1_wavelet_10kHz_bp1-80.dat" in fn:
