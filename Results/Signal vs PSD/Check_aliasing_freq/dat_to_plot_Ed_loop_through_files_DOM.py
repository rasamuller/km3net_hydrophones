...
def main():
    ...
    # DOM files
    files   = ['20170309_config1_DOM_7_toon_30kHz.dat', '20170309_config1_DOM_10_burst_30kHz.dat', '20170309_config2_DOM_17_sweep10-60kHz_10Vpp.dat', '20170310_config2_DOM_10_v1_wavelet_30kHz.dat']

    # all Rectangular window!    
#    files   = ['sine', 'Burst_30kHz_5Vpp_12cycles_hamming_AWG33220.dat', 'Swp_5-65kHz_5Vpp_Clock2MHz_windowed_AWG33220.dat', 'Wavelet_30kHz_5Vpp.dat']
...
#            time = np.linspace(0, length-1, length)/Fs
#            if 'Swp' in i:
#                plt.plot(time[0:1560]/(1e-3), data[0:1560]/(1e5), linewidth=1)
#            elif 'toon' in i:
#                plt.plot(time[0:int((0.51e-3*Fs))+1]/(1e-3), data[0:int((0.51e-3*Fs))+1]/(1e5), linewidth = 1)
#            elif 'burst' in i:
#                plt.plot(time[0:int(0.55e-3*Fs)+1]/(1e-3), data[int(31.35e-3*Fs):int(0.55e-3*Fs)+1+int(31.35e-3*Fs)]/(1e5), linewidth=1)
#            elif 'sweep' in i:
#                plt.plot(time[0:int(0.8e-3*Fs)+1]/(1e-3), data[int(31.2e-3*Fs):int(0.8e-3*Fs)+1+int(31.2e-3*Fs)]/(1e5), linewidth=1)
#            elif 'wavelet' in i:
#                plt.plot(time[0:int(0.2e-3*Fs)+1]/(1e-3), data[int(59.44e-3*Fs):int(0.2e-3*Fs)+1+int(59.44e-3*Fs)]/(1e5), linewidth=1)
#            else:
...
#            plt.clf()
    ...
    #########
    # NB: VOOR DE SPECTRA VAN ERWINS FILES, MOET JE EEN PAAR X DE TIMETTRACE HERHALEN OM ER EEN MOOIE FFT VAN TE MAKEN.
    #########
    
            freq = MakeFreqSpan(Fs, NFFT)
            if 'Swp' in i:        
                PSD = Spectrum(Fs, NFFT, data[0:1200], filename)[1]
            else:
                PSD = Spectrum(Fs, NFFT, data, filename)[1]
            plt.plot(freq, 20*np.log10(np.sqrt(PSD)), linewidth=1, label = i)
            plt.title('\n%s' %filename, fontsize=24)
            plt.tick_params(axis='both', labelsize=15)
            plt.xlabel('Frequency [Hz]', fontsize=18)
            plt.autoscale(tight = True)
